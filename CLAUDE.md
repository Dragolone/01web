...
- ✅ **表单 SMTP 已配置（2026-09-19）**：发信账号 QQ 邮箱 `810170966@qq.com`（授权码在服务器 `.env.local`，用户可在 QQ 邮箱「账号与安全」作废重发），收件 `CONTACT_TO=zhongqilong@01weichuang.com`（2026-09-19 起）。服务器到 `smtp.qq.com:465` 可达；从外部 POST `/api/contact` 返回 `{ok:true}`，测试邮件已发。不配 SMTP 时表单退回 mailto 的兜底逻辑仍在。**Gmail SMTP 从广州服务器大概率不通**（Google 域名在大陆不可达），建议 QQ 邮箱 `smtp.qq.com:465` + 授权码 或 腾讯企业邮，`CONTACT_TO` 仍可填 gmail。SMTP 变量是 runtime，`pm2 restart 01web` 即生效不用 build
