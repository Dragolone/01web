...
- ⏳ **表单 SMTP 待配置**：服务器 `.env.local` 填 `SMTP_HOST/SMTP_PORT/SMTP_USER/SMTP_PASS/CONTACT_TO` 后 `/api/contact` 即生效；不配则表单退回 mailto。**Gmail SMTP 从广州服务器大概率不通**（Google 域名在大陆不可达），建议 QQ 邮箱 `smtp.qq.com:465` + 授权码 或 腾讯企业邮，`CONTACT_TO` 仍可填 gmail。SMTP 变量是 runtime，`pm2 restart 01web` 即生效不用 build
