...
- ⏳ **公安联网备案进行中（2026-09-19 进度）**：平台 https://beian.mps.gov.cn ，数据码 `b2a747a66b31b0ea4206454ec634e1`（已绑定）。之前一版被驳回，原因「证件有效期填成长期」。9-19 用户已**重新提交开办主体**（新址坪山工匠园 402 / 负责人龙俊洁 / 身份证有效期 **2034-03-04**，三张身份证照片已传，主体已切换成功）。**网站备案（新办网站申请）填到第 02 步「网站基本信息」还没提交**，明天接着填，要点：网站名称与 ICP 一致「深圳零一唯创科技有限公司官网」；开通日期 2026-06-18；访问地址改成带 www；**域名证书**去 console.cloud.tencent.com/domain 下载，有效期 2026-05-31～2027-05-31，**域名服务商填 烟台帝思普网络科技有限公司（DNSPod, Inc.），境内 / 山东省烟台市**（证书 9-19 已下载，注册所有者＝公司）；**接入商＝腾讯云计算（北京）有限责任公司，北京市/市辖区/海淀区**（平台列表里腾讯只有这一家，深圳那家不对），接入方式「租赁虚拟空间」；市值留空（只有上市公司填）；**「互联网交互服务」「管制物品」「前置许可」三个都选否**（默认是「是」，选是会要许可证）；语种勾 简体+繁体+英语；第 03 步负责人选「同主体负责人信息」。提交后删掉「我的→我的草稿」里被驳回的旧草稿。审核结果短信发 135****6256。通过后把「粤公网安备 …号」发我加到 footer（链 beian.mps.gov.cn）。**ICP 备案里的地址可能还是旧址**，公安过了以后去腾讯云备案控制台做一次变更。
...
- ✅ **部署与搜索引擎推送工具（2026-09-19）**：
  ...
  - **`npm run notify`**（`scripts/notify-search-engines.mjs`，`node --env-file=.env.local`，服务器 Node 20.20 支持）：把 sitemap 里的 URL（或命令行给的 URL）推给 **IndexNow**（Bing/Yandex 等，key 由 `INDEXNOW_KEY` 提供，`src/app/indexnow.txt/route.ts` force-static 在 `/indexnow.txt` 输出它，请求里传 `keyLocation`）和 **百度 API 推送**（`BAIDU_PUSH_TOKEN`，只推 `/zh` 页面省配额）。缺哪个变量就跳过哪个，`--dry-run` 只打印。 2026-09-19 首推：IndexNow 200，百度 `400 site init fail`（站点刚验证、推送接口未初始化，属正常，每次 deploy 会自动重试；若数天后仍如此再查 site 参数是否与后台一致）。Google 没有推送接口，靠 sitemap。**`INDEXNOW_KEY` 是 build-time**（route 预渲染），改了要重新 build；`BAIDU_PUSH_TOKEN` 是 runtime。
